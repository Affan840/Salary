...
app.post('/api/estimated-salary', async (req, res) => {
    ...
    const estimatedSalary = await getEstimatedSalary(jobTitle, location, yearsOfExperience);
...
async function getSalaryFromLinkedIn(jobTitle, yearsOfExperience) {
    const url = `https://linkedin-data-api.p.rapidapi.com/search-jobs-v2?keywords=${encodeURIComponent(jobTitle)}&locationId=92000000&experienceLevel=${yearsOfExperience}&datePosted=anyTime&sort=mostRelevant`;
    const options = {
        method: 'GET',
        headers: {
...
async function getJobDataFromGlassdoor(query) {
    const url = `https://glassdoor-real-time.p.rapidapi.com/jobs/search?query=${encodeURIComponent(query)}`;
    const options = {
        method: 'GET',
        headers: {
            'x-rapidapi-key': '292ab28bcdmshb7ddb7eed20f132p130835jsn66c9bef19630', // Replace with your actual API key
